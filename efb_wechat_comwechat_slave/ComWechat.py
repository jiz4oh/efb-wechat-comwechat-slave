        ...
        if isinstance(msg.target, Message):
                if isinstance(msg.target.author, SelfChatMember) and isinstance(msg.target.deliver_to, SlaveChannel):
                    ...
                    if "@chatroom" in msg.author.chat.uid:
                        xml = QUOTE_GROUP_MESSAGE % (text, msgid, sender, msg.author.chat.uid, self.wxid, displayname, content)
                    else:
                        xml = QUOTE_MESSAGE % (text, msgid, sender, self.wxid, displayname, content)
